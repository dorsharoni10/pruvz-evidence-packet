...
| `disagreeing-exact-amount.packet.json` | `decision.amount` displays `42.5` but states `amountExact: "4.25"`. | The two representations of one amount must denote the same value (format 1.4.0). A cryptographic commitment binds the exact string, so a packet that displays one amount and commits another would otherwise verify while misleading every human reader. |
