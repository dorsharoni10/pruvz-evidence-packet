...
const requireTimestampingPurpose = (certificate) => {
  ...
  if (!extension) {
    fail('ANCHOR_AUTHORITY_UNTRUSTED', 'the signing certificate declares no extended key usage')
  }
  if (extension.critical !== true) {
    fail(
      'ANCHOR_AUTHORITY_UNTRUSTED',
...
export async function verifyTimestampAuthority({ token, roots, imprintInput, policyOids = null } = {}) {
  ...
  const chainEngine = new pkijs.CertificateChainValidationEngine({
    certs: (signedData.certificates ?? []).filter((entry) => entry instanceof pkijs.Certificate),
    trustedCerts,
    checkDate: genTime,
  })
  ...
  if (signatureVerifies !== true) {
    fail('ANCHOR_RECEIPT_SIGNATURE_INVALID', "the token's CMS signature does not verify")
  }
